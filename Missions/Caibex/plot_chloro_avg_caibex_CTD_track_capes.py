#!/usr/bin/env python
#
# plot_chloro_avg_caibex_CTD_track_capes.py
#
# Plot the averaged chlorophyll concentration, the CTD track and the cape names
#
...
# Allocate
chloromatrix=np.zeros((nfiles,len(lat),len(lon)))
...
chloroavg=stats.nanmean(chloromatrix)
...
if doplot==1:
    ...
    # Mask land values
    chloroavg = np.ma.masked_array(chloroavg,np.isnan(chloroavg))
    ...
    pcm=m.pcolormesh(x,y,chloroavg,cmap=cmap,norm=norm)
