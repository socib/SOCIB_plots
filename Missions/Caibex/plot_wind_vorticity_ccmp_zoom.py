#!/usr/bin/env python
#
# plot_wind_vorticity_ccmp_zoom.py
# 
...
figdir2='/home/ctroupin/DataOceano/Wind/CCMP/figures/windcurl_zoom/'
...
if doplotmean ==1:
    ...
    fig=plt.figure()
    ax = fig.add_subplot(111)
    plt.plot(vortmean,'ko-',linewidth=2)
    plt.title('Averaged wind curl (normalized by f)',fontsize=24,fontname='Times New Roman')
    xticklabels=ax.get_xticks()
    ax.set_xticklabels('')
    yticklabels=ax.get_yticks()
    ax.set_yticklabels(yticklabels,fontsize=18,fontname='Times New Roman')
    ax.set_xlim(0,nfiles-1)
    ax.set_ylim(0,0.31)
    plt.savefig(figdir2+'windcurl_avg.png', dpi=300, facecolor='w', edgecolor='w', pad_inches=0.1)
    plt.show()
    plt.close()
